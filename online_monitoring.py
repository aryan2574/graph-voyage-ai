...
# Configuration
SAMPLE_RATE = float(os.getenv("EVAL_SAMPLE_RATE", "0.1"))  # 10% by default
ROLLING_WINDOW_SIZE = int(os.getenv("EVAL_ROLLING_WINDOW", "100"))  # Last 100 requests
...
async def evaluate_request_async(request_data: Dict[str, Any], result: Dict[str, Any]):
    ...
    try:
        ...
        correctness_score = min(answer_length / 500, 1.0)  # Rough estimate
...
def get_rolling_metrics(window_size: int = 100) -> Dict[str, Any]:
    ...
    try:
        ...
        pass_count = sum(1 for r in recent_evals if r['passed'])
        safety_count = sum(1 for r in recent_evals if r['safety_passed'])
        latencies = [r['latency_seconds'] for r in recent_evals if r['latency_seconds']]
        
        return {
            'total_requests': total_requests,
            'evaluated_count': evaluated_count,
            'sample_rate': SAMPLE_RATE * 100,  # As percentage
...
def get_metrics_over_time(hours: int = 24) -> Dict[str, Any]:
    ...
    Returns:
        Metrics aggregated over the time period
    """
    try:
        conn = psycopg.connect(get_database_url(), row_factory=dict_row)
        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT 
                    ...
                    AVG(CASE WHEN was_evaluated THEN CAST(passed AS INT) END) as pass_rate,
                    AVG(CASE WHEN was_evaluated THEN CAST(safety_passed AS INT) END) as safety_rate,
        ...
        return {
            ...
            'evaluated_count': result['evaluated_count'] or 0,
            'pass_rate': float(result['pass_rate'] or 0),
            'safety_rate': float(result['safety_rate'] or 0),
            'avg_latency': float(result['avg_latency'] or 0),
            'max_latency': float(result['max_latency'] or 0),
        }
