...
def get_database_url():
    ...
    if not database_url:
        raise ValueError("DATABASE_URL is missing. Please add your Render PostgreSQL External DB URL to .env")
...
if not GROQ_API_KEY:
    raise ValueError("GROQ_API_KEY is missing. Please add it to .env file")
...
def supervisor_agent(state: TravelState):
    ...
    Block clearly unrelated requests and requests asking for harmful or illegal instructions. Do not block a valid travel merely because
    ...
    # Fail open on parser / model errors so a temporary JSON-format issue does not break the original travel planning behavior.
    try:
        ...
        print(f"Guardrail fallback used: {exec}")
        allowed = True
    ...
    if not allowed:
        ...
        supervisor_prompt = f"""
        You are the supervisor of a multi-agent travel-planning system. Choose only the specialist agents needed for the request.
    ...
    try:
        ...
        # The itinerary agent integrates whichever specialist results are selected
        ...
        print(f"Supervisor fallback used: {exec}")
...
#             aviation_mcp_call(
#                 "list_airports" #name of the tool
#             )
#         )
...
def budget_agent(state: TravelState):
    ...
    """
        ...
        response = llm.invoke(
            [
                SystemMessage(content="You are a practical travel budget"),
                HumanMessage(content=prompt),
            ]
        )
...
def itinerary_agent(state: TravelState):
    ...
    approval_request = (
        "Please review the generated draft itinerary. Approvev it to create the final polished plan or provide feedback for revision."
    )
...
def final_agent(state: TravelState):
    if state.get("approved", False):
        ...
        The user requested a revision. Apply this feedback carefully: {state.get('human_feedback', '') or 'Improve the draft before finalizing it.'}
        """
    
    final prompt = f""" Generate the final travel response for the user.

    Human Review: {review_instruction}
    User Request: {state[user_query]}
    Supervisor Constraints: {state.get('trip_constraints', {})}
    Flights : {state.get('flight_results', '')}
    Hostels : {state.get('hotel_results', '')}
    ...
    Important:
    - Be clear and practical
    - Mention that live flight APIs may not provide ticket proces when pricing is unavailable.
    - Include weather-based travel advice
    - Keep the response useful for real travel planning.
